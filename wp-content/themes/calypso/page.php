	...
	<!-- IF NO TOP SHORTCODE ADDED -->
		<div id="pagephp page-<?php the_ID(); ?>">
			...
			if( $active_pageheader != '' ) { ?>
									...
									<?php 
										if( $no_title != 'on' ) { ?>
											<h2 class="title wow <?php echo $header_titleanim;?>" wow-data-duration="0.8s">	
												<div style="font-size:<?php echo $titlesize; ?>;display: inherit;"><?php echo get_the_title();?></div>
											</h2> <?php	}
										...
										if( $header_downarrow != '' ){ ?>
										<a href="#scrolltopost" class="downarrowpoint wow zoomIn goscroll"><i class="fa fa-angle-double-down"></i></a>
										<?php } 
									?>										
								</div> <!-- .section-title -->
				...
				<section class="pageheaderpagephp parallax parallax-image" style="background-image:url(<?php echo $bbwoptions['pageheader_upload']; ?>);">	
					<div class="wrapsection" style="padding:<?php echo $bbwoptions['pageheader_padding']; ?> 0px;">
						<div class="overlay" style="background-color:<?php echo $bbwoptions['pageheader_background_color']; ?> ; opacity: <?php echo $bbwoptions['pageheaderopacity_text']; ?>;"></div>
							<div class="container">
							<div class="parallax-content">
								<div class="title-area">
									<h2 class="title wow zoomIn">
										<span style="color: <?php echo $bbwoptions['pageheader_font_style']['color']; ?>;	font-size: <?php echo $bbwoptions['blogindexheader_font_style']['size']; ?>;display: inherit;"><?php echo get_the_title();?></span>
									</h2>
								</div> <!-- .section-title -->																		
							</div> <!-- .parallax-content -->
						</div>						
				...
				<div class="wrapsemibox">
					...
					<div class="container" id="scrolltopost">
						...
						endwhile;		
						wp_reset_query();
						if( !empty( $parallaxId ) )	{wp_enqueue_script( 'wowparallax' );} ?>
						<div class="clearfix">
							<?php
								if ( comments_open() || '0' != get_comments_number() )
								comments_template();
